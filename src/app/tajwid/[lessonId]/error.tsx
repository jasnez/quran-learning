...
export default function TajwidLessonError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Tajwid lesson error:", error.message, error.digest);
  }, [error]);

  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 py-12">
      <h1 className="text-xl font-semibold text-stone-900 dark:text-stone-100">
        Greška pri učitavanju lekcije
      </h1>
      <p className="mt-2 max-w-md text-center text-sm text-stone-600 dark:text-stone-400">
        Nije moguće učitati ovu tajwid lekciju. Provjeri internet vezu i pokušaj ponovo.
      </p>
      {error.digest && (
